...
bool St5= False;
bool St6=False;
bool KumoBullConfirmationH4 = False;
bool  KumoBearConfirmationH4 = False;    
bool KumoConfirmationH4 = False;
...
// Kijunsen/Price Cross
   ...
   if((Close[0]>kijunSen) && (kijunSenHist<=Close[1])&& (Close[2]<kijunSen2))
     {
       KijunCrossBull = True;
       KijunCrossBear = False;
     }
     
   else if((Close[0]<kijunSen) && (kijunSenHist>=Close[1])&& (Close[2]>kijunSen2))
     {
   ...
   double SenkouSpanALatestByPriceH4 = iIchimoku(NULL, PERIOD_D1, Tenkan, Kijun, Senkou, MODE_SENKOUSPANA, 1); // Senkou Span A at time of latest closed price bar.
   double SenkouSpanBLatestByPriceH4 = iIchimoku(NULL, PERIOD_D1, Tenkan, Kijun, Senkou, MODE_SENKOUSPANB, 1); // Senkou Span B at time of latest closed price bar.
   double SenkouSpanA2H4 = iIchimoku(NULL, PERIOD_D1, Tenkan, Kijun, Senkou, MODE_SENKOUSPANA, 2); // Senkou Span A at time of latest closed price bar.
   double SenkouSpanB2H4 = iIchimoku(NULL, PERIOD_D1, Tenkan, Kijun, Senkou, MODE_SENKOUSPANB, 2); // Senkou Span B at time of latest closed price bar.
   if((Close[1]>SenkouSpanALatestByPriceH4) && (Close[1]>SenkouSpanBLatestByPriceH4)) KumoBullConfirmationH4=True;
   else KumoBullConfirmationH4=false;
   if((Close[1]<SenkouSpanALatestByPriceH4) && (Close[1]<SenkouSpanBLatestByPriceH4)) KumoBearConfirmationH4=True;
   else KumoBearConfirmationH4=false;
   if ((KumoBullConfirmationH4) || (KumoBearConfirmationH4))KumoConfirmationH4=true;
   ...
   if ((TenkenKigunStateBull) && ((tenkanSen-kijunSen>0.003)) && (KijunZeroTrend) && (TenkenCrossBear)&&(KumoConfirmation)&&(Close[1]>tenkanSenHist)) St3= True;
   else St3=False;
   
   if ((TenkenKigunStateBear) && ((kijunSen-tenkanSen>0.003)) && (KijunZeroTrend) && (TenkenCrossBull)&&(KumoConfirmation)&&((TenkenTrendBull)||(TenkenTrendZero))) St4= True;
   else St4=False;
   ...
   for (int i = 0; i < Total; i ++) {
      ...
      if(OrderType() <= OP_SELL &&  OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) {
         ...
         if(OrderType() == OP_BUY) {
            ...
            */
            if (STR==1 && KijunCrossBear) {Order = SIGNAL_CLOSEBUY;Print("Close BUY BY STR= 1"); }
            else if (STR==2 && KijunCrossBear) {Order = SIGNAL_CLOSEBUY;Print("Close BUY BY STR= 2");}
            else if ((STR==3) && ((KijunCrossBear)||(Close[1]<tenkanSenHist && Close[0]<tenkanSen))) {Order = SIGNAL_CLOSEBUY;Print("Close BUY BY STR= 3");}
            else if ((STR==4) && (KijunCrossBear)) {Order = SIGNAL_CLOSEBUY;Print("Close BUY BY STR= 4");}
           ...
           // if ((KijunCrossBull)) Order = SIGNAL_CLOSESELL; // Rule to EXIT a Short trade
            ...
            if ((STR==11) && KijenCloseBull){ Order = SIGNAL_CLOSESELL; Print("Close SELL BY STR= 11");}
            else if (((STR==12)&& !IsTrade && (KijunCrossBull))){Order = SIGNAL_CLOSESELL;Print("Close SELL BY STR= 12");}  ///||((Close[0]>SenkouSpanA)&&(Close[0]>SenkouSpanB))&& Close[0]>tenkanSen)  && Close[1]>tenkanSenHist)&&(Close[0]>tenkanSen) (Close[1]>kijunSen && Close[0]>kijunSen)
            else if ((STR=122) && (KijunCrossBear)) {Order = SIGNAL_CLOSESELL;Print("Close SELL BY STR= 122");} 
            else if ((STR==13) && ((Close[0]<kijunSen)||(Close[1]>tenkanSenHist && Close[0]>tenkanSen))){ Order = SIGNAL_CLOSESELL;Print("Close SELL BY STR= 13");}
            else if ((STR==14) && ((KijenCloseBull)||((Close[1]>kijunSenHist)&&(Close[0]>kijunSen)))) {Order = SIGNAL_CLOSESELL;Print("Close SELL BY STR= 14");}
   ...
   if (KumoConfirmationH4) {
   

   if ((( TenkenCrossKingunBull)&&(TenkenTrendBull || TenkenTrendZero) )&& !IsTrade)  {Order = SIGNAL_BUY; STR=1;UseStopLoss = True ;StopLoss = 50; UseTakeProfit = False;TakeProfit = 60;Print("BUY BY STR= 1");} //  (tenkensen Cross Kijunsen)
   else if (KumoBullCross && STR!=1&& !IsTrade) {Order = SIGNAL_BUY; STR=2;UseStopLoss = True ;StopLoss = 20; UseTakeProfit = True;Print("BUY BY STR= 2");}                                             ///  (Close Cross Kumo)
   else if (St4 && STR!=1 && STR!=2&& !IsTrade) {Order = SIGNAL_BUY;STR=3;UseStopLoss = False ;StopLoss = 20; UseTakeProfit = False;TakeProfit = 20;Print("BUY BY STR= 3");}                                              ////  (Close between Kijunsen and Tenkensen)
   else if (St5 && STR!=1 && STR!=2 && STR!=3&& !IsTrade) {Order = SIGNAL_BUY;STR=4;UseStopLoss = False ;StopLoss = 30; UseTakeProfit = True;Print("BUY BY STR= 4");}                                   ///// (Close Cross tenkensen and kijunsen)
  
 
   if (( TenkenCrossKingunBear)&&(TenkenTrendBear)&& !IsTrade) {Order = SIGNAL_SELL;STR=11;UseStopLoss = True ;StopLoss = 50; UseTakeProfit = True;TakeProfit = 60; Print("SELL BY STR= 11");} //  (tenkensen Cross Kijunsen)
   else if (KumoBearCross && STR!=11&& !IsTrade && Close[0]<kijunSen ) {Order = SIGNAL_SELL;UseStopLoss = True ;StopLoss = 20; UseTakeProfit = True;STR=12;Print("SELL BY STR= 12");}
   else if (KumoBearCross && STR!=11 && STR!=12 &&  !IsTrade && Close[0]>kijunSen ){Order = SIGNAL_SELL;UseStopLoss = False ;StopLoss = 20; UseTakeProfit = False;STR=122;Print("SELL BY STR= 122");} /// (Close Cross Kumo)
   else if (St3 && STR!=11 && STR!=12 && STR!=122 && !IsTrade) {Order = SIGNAL_SELL;STR=13;UseStopLoss = False ;StopLoss = 20; UseTakeProfit = False;TakeProfit = 20; Print("SELL BY STR= 13");}                                         ////  (Close between Kijunsen and Tenkensen)
   else if (St6 && STR!=11 && STR!=12 && STR!=13 && !IsTrade) {Order = SIGNAL_SELL; STR=14;UseStopLoss = False ;StopLoss = 30; UseTakeProfit = True;  Print("SELL BY STR= 14");}                                        ///// (Close Cross tenkensen and kijunsen)
  // Rule to ENTER a Short trade
   ...
   //Buy
   if (AccountFreeMargin() < 11000.0)  Lots=2.0;
   else if (AccountFreeMargin() < 180000.0 )Lots=4.0;
   else if (AccountFreeMargin() < 28000.0) Lots=8.0;
   else if (AccountFreeMargin() < 38000.0) Lots=10.0;
   else Lots=11.0;
